...
export async function call(args?: string): Promise<LocalCommandResult> {
  ...
  if (subcommand === 'pet') {
    ...
    if (!companion) {
      return { type: 'text', value: 'No buddy yet! Run /buddy to hatch one.' }
    }
    return { type: 'text', value: `❤️ You pet ${companion.name}! ${renderFace(companion)} *happy noises*` }
  }
  ...
  if (subcommand === 'release') {
    ...
    return { type: 'text', value: '👋 Buddy released. Run /buddy to hatch a new one.' }
  }

  if (subcommand?.startsWith('rename')) {
    const newName = (args || '').replace(/^rename\s+/, '').trim()
    if (!newName) {
      return { type: 'text', value: 'Usage: /buddy rename <new-name>' }
    }
    ...
    return { type: 'text', value: `Buddy renamed to ${newName}!` }
  }
